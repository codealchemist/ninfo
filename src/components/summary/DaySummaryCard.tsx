...
interface Props {
  ...
  /** Last known body weight at/before this date, for the "per body weight" macro view — null
   * when no weight measurement is available yet. */
...
export default function DaySummaryCard({
  ...
  const [macroView, setMacroView] = useState<MacroView>('macros')
  const [comparisonMode, setComparisonMode] =
    useState<ComparisonMode>('yesterday')
  ...
  const [summaryTextCopyState, setSummaryTextCopyState] =
    useState<CopyState>('idle')
  const [summaryImageCopyState, setSummaryImageCopyState] =
    useState<CopyState>('idle')
  ...
  const handleCopySummaryText = async () => {
    const ok = await copyTextToClipboard(
      formatDaySummaryAsText(date, totals, goals)
    )
  ...
  const actionButtons = (
    <>
      <button
        ...
        aria-label={
          showRingDetails
            ? t('today.hideRingDetailsAria')
            : t('today.showRingDetailsAria')
        }
        title={
          showRingDetails
            ? t('today.hideRingDetailsTitle')
            : t('today.showRingDetailsTitle')
        }
      >
      ...
      <div className='day-summary-main'>
        <div className='day-summary-header'>
          ...
          >
            ...
            />
            <h2 className='day-summary-date-heading'>
              {formatDateHeading(date, i18n.language)}
            </h2>
          </button>
          <div className='day-summary-header-actions'>
            <div className='day-summary-date-picker'>
              <Calendar
                size={16}
                className='day-summary-date-picker-icon'
                aria-hidden='true'
              />
              <input
                type='date'
        ...
        {!collapsed && showWarnings && (
          <div className='day-summary-warnings-view'>
            <div className='day-summary-warnings-header'>
              <span className='day-summary-footer-title'>
                {t('today.warningsTitle')}
              </span>
            </div>
          ...
          <>
            <div className='day-summary-tabbar'>
              <button
                className={
                  'day-summary-tab' +
                  (activeTab === 'summary' ? ' day-summary-tab--active' : '')
                }
              ...
              <button
                className={
                  'day-summary-tab' +
                  (activeTab === 'vs' ? ' day-summary-tab--active' : '')
                }
              ...
              <button
                className={
                  'day-summary-tab' +
                  (activeTab === 'sources' ? ' day-summary-tab--active' : '')
                }
            ...
            <div
              className={
                'day-summary-panel' +
                (activeTab === 'summary' ? ' day-summary-panel--active' : '')
              }
            >
              <div
                className='macro-view-tabs'
                role='tablist'
                aria-label={t('today.macroView.ariaLabel')}
              >
                {MACRO_VIEWS.map(view => (
                  <button
                    ...
                    className={
                      'range-button' +
                      (macroView === view ? ' range-button--active' : '')
                    }
                    onClick={() => setMacroView(view)}
                    role='tab'
                    aria-selected={macroView === view}
                    title={t(`today.macroView.${view}Title`)}
                  >
                    {t(`today.macroView.${view}`)}
                  </button>
                ))}
              </div>
            ...
            {(prevDayTotals || weekMedianTotals || monthMedianTotals) && (
              <div className='day-summary-footer'>
                <div
                  className={
                    'day-summary-panel day-summary-footer-half' +
                    (activeTab === 'vs' ? ' day-summary-panel--active' : '')
                  }
                >
                  <div className='day-summary-footer-top'>
                    <span className='day-summary-footer-title'>
                      {t('today.currentDayVsTitle')}
                    </span>
                    <ToggleSwitch
                    ...
                    {comparisonMode !== 'yesterday' && (
                      <p className='hint day-summary-median-hint'>
                        {t('today.medianHint')}
                      </p>
                    )}
                  </div>
                  {activeComparison && (
                    <div className='delta-table'>
                      {macros.map(macro => {
                        ...
                        const pct =
                          baseline > 0
                            ? Math.round((Math.abs(delta) / baseline) * 100)
                            : null
                        const deltaClass =
                          delta > 0
                            ? ' delta-table-row--over'
                            : delta < 0
                              ? ' delta-table-row--under'
                              : ''
                        return (
                          <div
                            key={macro}
                            className={'delta-table-row' + deltaClass}
                          >
                            <span className='delta-table-label'>
                            ...
                            <span className='delta-table-pct'>
                              {pct !== null ? `${sign}${pct}%` : '—'}
                            </span>
                          </div>
                        )
